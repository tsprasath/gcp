...
resource "google_compute_firewall" "private-allow-ssh" {
  name    = "${google_compute_network.vpc_private.name}-allow-ssh"
...
resource "google_compute_firewall" "private-allow-rdp" {
  name    = "${google_compute_network.vpc_private.name}-allow-rdp"
  network = google_compute_network.vpc_private.name
  allow {
    protocol = "tcp"
    ports    = ["3389"]
...
resource "google_compute_firewall" "private-allow-ping" {
  name    = "${google_compute_network.vpc_private.name}-allow-ping"
  network = google_compute_network.vpc_private.name
  allow {
    protocol = "icmp"
  }
